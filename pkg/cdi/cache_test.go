...
func TestNewCache(t *testing.T) {
	...
	for _, tc := range []*testCase{
		...
		{
			name:   "no spec dirs",
			option: WithCreateMissingDirs(true),
		},
		{
			name: "no spec files",
			etc:  map[string]string{},
				...
				for idx, update := range tc.updates {
					if idx == 0 {
						...
						opts = []Option{
							...
							WithCreateMissingDirs(true),
						}
						if !selfRefresh {
							opts = append(opts, WithSelfRefresh(false))
						}
						cache, err = NewCache(opts...)
						require.NotNil(t, cache)
					} else {
...
`,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Run Spec update loop until an error request to stop.
			updater := func() {
				var (
					idx = 1
					err error
				...
				for {
					...
					// try to amortize major fs-hit from massive create+write+rename
					syscall.Sync()
...
func touchFiles(dir string, files []string) error {
	for _, file := range files {
		path := filepath.Join(dir, file)
		err := os.MkdirAll(filepath.Dir(path), 0o755)
		if err != nil {
			return errors.Wrapf(err, "failed to touch file %s", path)
		}
